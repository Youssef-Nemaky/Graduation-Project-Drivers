...
#if (DIO_CONFIGURED_GROUP_CHANNELS != 0)
STATIC const Dio_ChannelGroupType * Dio_GroupChannels = NULL_PTR;
#endif
...
#if (DIO_DEV_ERROR_DETECT == STD_ON)
    if(PortId >= GPIO_PORT_NUMS)
    {
        /* Report to DET  */
        Det_ReportError(DIO_MODULE_ID, DIO_INSTANCE_ID,
                        DIO_GET_VERSION_INFO_SID, DIO_E_PARAM_INVALID_PORT_ID);
    }
    else
#endif
    {
